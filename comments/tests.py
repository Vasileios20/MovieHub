...
class TestViews(TestCase):
    ...
    def test_get_comments_page(self):
        response = self.client.get(reverse('comment_movie', args=[1]))
        self.assertEqual(response.context['commented'], False)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'comments.html')

    def test_comment_movie(self):
        response = self.client.post(reverse('comment_movie', args=[1]), {
            'comment': 'test comment'
        })
        self.assertEqual(response.status_code, 302)

    def test_edit_comment(self):
        response = self.client.post(reverse('edit_comment', args=[1, 1]), {
            'comment': 'test comment'
        })
        self.assertEqual(response.status_code, 302)

    def test_delete_comment(self):
        response = self.client.post(reverse('delete_comment', args=[1, 1]))
        self.assertEqual(response.status_code, 302)
