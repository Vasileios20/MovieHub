...
def ratings_list():
    ...
    for movie_id in temp:
        ...
        temp_average.append({"average": average, "movie_id": movie_obj.movie_id})
    # Sort the list by the average rating
    temp_average = sorted(temp_average, key=lambda d: d["average"], reverse=True)
    ...
    for movie_id in temp_average:
        ...
        data.update({"movie_obj": movie_obj.movie_id, "width": width})
...
def add_rating(request):
    ...
    if request.method == "POST":
        ...
        if not rate.exists():
            ...
            messages.add_message(request, messages.SUCCESS, "Rating added successfully")
